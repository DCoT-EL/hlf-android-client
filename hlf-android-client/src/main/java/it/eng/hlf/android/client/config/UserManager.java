...
public class UserManager {
    ...
    public void completeUsers(InputStream cert, InputStream keystore) throws HLFClientException {
        try {
            Set<User> users = organization.getUsers();
            for (User user : users) {
                doCompleteUser(user, cert, keystore);
            }
    ...
    private void doCompleteUser(User user, InputStream cert, InputStream keystore) throws IOException, NoSuchProviderException, NoSuchAlgorithmException, InvalidKeySpecException, HLFClientException {
        ...
        String certificate = new String(IOUtils.toByteArray(cert), ConfigManager.UTF_8);
        //File fileSk = Utils.findFileSk(organization.getDomainName(), user, configuration.getCryptoconfigdir());
        PrivateKey privateKey = Utils.getPrivateKeyFromBytes(IOUtils.toByteArray(keystore));
